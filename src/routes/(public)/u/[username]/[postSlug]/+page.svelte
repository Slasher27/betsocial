...
<svelte:head>
	<title>{data.post.title} - {data.author.display_name || data.author.username} | BetChat</title>
</svelte:head>
	...
	<article class="container mx-auto px-4 py-16 max-w-2xl">
		...
		<header class="mb-12">
			...
			<div class="flex items-center gap-4 mb-8">
				<a href="/u/{data.author.username}" class="flex items-center gap-3 group">
					<div class="avatar">
						<div class="w-12 h-12 rounded-full">
							{#if data.author.avatar_url}
								<img src={data.author.avatar_url} alt={data.author.username} class="object-cover" />
							{:else}
								<div class="w-full h-full bg-neutral text-neutral-content flex items-center justify-center">
									<span class="font-semibold">{data.author.username[0].toUpperCase()}</span>
								</div>
							{/if}
						</div>
					</div>
			...
			<div class="flex items-center gap-6 py-4 border-y border-base-300 text-sm">
				...
				<ShareMenu
					url={typeof window !== 'undefined' ? window.location.href : `https://betchat.social/u/${data.author.username}/${data.post.slug}`}
