...
<svelte:head>
	<title>{data.profile.display_name || data.profile.username} | BetChat</title>
</svelte:head>
	...
	<div class="container mx-auto px-4 max-w-[680px]">
		...
		<div class="relative w-full h-[240px] mt-12 mb-0 overflow-hidden rounded-lg">
			{#if data.profile.banner_url}
				<img src={data.profile.banner_url} alt="{data.profile.username} banner" class="w-full h-full object-cover" />
			{:else}
		...
		<div class="pb-8 border-b border-base-300/60 md:px-2">
				...
				<div class="avatar">
					<div class="w-24 h-24 rounded-full ring-4 ring-base-100 bg-base-100">
						{#if data.profile.avatar_url}
							<img src={data.profile.avatar_url} alt={data.profile.username} class="object-cover rounded-full" />
						{:else}
							<div class="w-full h-full bg-neutral text-neutral-content flex items-center justify-center rounded-full">
								<span class="text-4xl font-bold">{data.profile.username[0].toUpperCase()}</span>
							</div>
						{/if}
					</div>
				</div>
			...
			<div class="mb-5">
				...
				<p class="text-base opacity-50">@{data.profile.username}</p>
			</div>

			<!-- Bio/Description -->
			{#if data.profile.bio}
				<p class="text-lg leading-relaxed mb-6 opacity-80">{data.profile.bio}</p>
			{/if}

			<!-- Social Links & Website -->
			{#if data.profile.website_url || data.profile.twitter_url || data.profile.facebook_url || data.profile.instagram_url || data.profile.linkedin_url || data.profile.youtube_url || data.profile.tiktok_url || data.profile.substack_url}
				<div class="flex flex-wrap items-center gap-2 mb-5">
					...
					{/if}
					{#if data.profile.facebook_url}
						<a
							href={data.profile.facebook_url}
							target="_blank"
							rel="noopener noreferrer"
							class="badge badge-lg gap-1.5 bg-base-300 hover:bg-base-content hover:text-base-100 transition-all border-0"
						>
					...
					{/if}
					{#if data.profile.tiktok_url}
						<a
							href={data.profile.tiktok_url}
							target="_blank"
							rel="noopener noreferrer"
							class="badge badge-lg gap-1.5 bg-base-300 hover:bg-base-content hover:text-base-100 transition-all border-0"
						>
					...
					{#if data.profile.substack_url}
						<a
							href={data.profile.substack_url}
							target="_blank"
							rel="noopener noreferrer"
							class="badge badge-lg gap-1.5 bg-base-300 hover:bg-base-content hover:text-base-100 transition-all border-0"
						>
			...
			<div class="flex items-center gap-3 text-sm opacity-50">
				<span><span class="font-semibold text-base-content opacity-100">{data.posts.length}</span> posts</span>
				<span>·</span>
				<span>
					Joined {new Date(data.profile.created_at).toLocaleDateString('en-US', {
						month: 'long',
						year: 'numeric'
					})}
				</span>
				...
				{#each filteredPosts as post}
					<article class="border-b border-base-300/50 pb-10 last:border-0">
						...
						<div class="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 mb-4">
							<a href="/u/{data.profile.username}/{post.slug}" class="block group">
							...
							{#if post.cover_image_url}
								<div class="w-full md:w-40 h-40 flex-shrink-0">
									<a href="/u/{data.profile.username}/{post.slug}" class="block h-full group">
										<img
		...
		{:else}
			<div class="text-center py-24 md:px-2">
				...
				<h3 class="text-2xl font-bold mb-2 opacity-70">
					{#if data.posts.length > 0 && selectedType !== 'all'}
